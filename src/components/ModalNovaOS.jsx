...
export default function ModalNovaOS({ isOpen, onClose, onSuccess }) {
  // 1. Identificação
  ...
  const resetarCampos = () => {
    ...
    setStatus('Pendente');
    setObservacoes('');
    setTipoAssinatura('manual'); // Reset da assinatura
  };
  ...
  const handleSalvarOS = async () => {
    ...
    const novaOS = {
      ...
      status,
      observacoes,
      tipo_assinatura: tipoAssinatura // Guarda a preferência na base de dados
    };

    // Usamos o .select() para retornar o ID da O.S. recém-criada
    ...
    if (error) {
      ...
      // Passa todos os dados necessários (incluindo a assinatura) de volta para o ecrã de Produção
      if (onSuccess) {
        onSuccess({ 
