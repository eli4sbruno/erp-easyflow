...
export default function Configuracoes() {
  ...
  // Estados do Formulário - Financeiro & Fiscal
  ...
  const handleSalvarGeral = async () => {
    ...
    if (error) {
      ...
      alert('Erro ao guardar as configurações. Verifique se adicionou as novas colunas no Supabase.');
    } else {
  ...
  const handleSalvarFinanceiro = async () => {
    ...
    setIsSaving(false);
    if (error) {
      console.error('Erro ao guardar configurações financeiras:', error);
      alert('Erro ao guardar as configurações. Verifique se adicionou as colunas financeiras no Supabase.');
    } else {
      setMensagemSucesso(true);
      setTimeout(() => setMensagemSucesso(false), 3000);
    }
        ...
        <div className="flex-1 w-full">
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button className="px-4 py-2 bg-slate-100 rounded-lg text-sm font-medium hover:bg-slate-200 transition-colors">Upload</button>
                    </div>
                  </div>
                  <div>
                    ...
                    <input 
                      ...
                      placeholder="******" 
                      className="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:outline-none focus:border-[#0F4C81] focus:ring-1 focus:ring-[#0F4C81]" 
                    />
                  </div>
                  <div className="md:col-span-2">
                    ...
                    <select 
                      ...
                      className="w-full border border-slate-300 rounded-lg p-2.5 text-sm bg-white focus:outline-none focus:border-[#0F4C81] focus:ring-1 focus:ring-[#0F4C81]"
                    >
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    >
                      ...
                      <option value="Inter Bank API">Inter Bank API</option>
                    </select>
                  </div>
                  <div>
                    ...
                    <input 
                      ...
                      placeholder="Insira o seu token de integração" 
                      className="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:outline-none focus:border-[#0F4C81] focus:ring-1 focus:ring-[#0F4C81]" 
                    />
                  </div>
                </div>

                {/* Rodapé de Salvar Específico da Aba Financeiro */}
                <div className="p-6 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
                  <div>
                    {mensagemSucesso && (
                      <span className="text-[#1B9C85] font-medium flex items-center gap-2 animate-fade-in text-sm">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
                        Configurações financeiras salvas!
                      </span>
                    )}
                  </div>
                  <button 
                    ...
                    className="px-6 py-2.5 bg-[#0F4C81] text-white text-sm font-medium hover:bg-[#0c3e6a] rounded-lg shadow-sm transition-colors disabled:opacity-70 flex items-center gap-2"
                  >
                    {isSaving ? 'A guardar...' : 'Guardar Configurações Financeiras'}
                  </button>
                </div>
              </div>
          ...
          {abaAtiva === 'Integracoes' && (
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-8 text-center animate-fade-in">
              ...
              <p className="text-slate-500 text-sm max-w-md mx-auto mb-6">
                Conecte o seu ERP a ferramentas como WhatsApp Business API, WooCommerce, Shopify, ou sistemas de marketing para automatizar os fluxos.
              </p>
